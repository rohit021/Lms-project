import React, { Component } from 'react';
import AppBar from '@material-ui/core/AppBar';
import { ThemeProvider as MuiThemeProvider } from '@material-ui/core/styles';
import { Grid, MenuItem, DialogContent, TextField, IconButton, makeStyles, Dialog} from '@material-ui/core';
import DialogTitle from '@material-ui/core/DialogTitle'
import Button from '@material-ui/core/Button';
import CloseIcon from '@material-ui/icons/Close';
import FormControl from '@material-ui/core/FormControl';
import { SourceOptions, CenterOptions, PriorityOptions, DepartmentOptions, PropertyNameOptions, CategoryOptions} from "../../helpers/utils";
const useStyles = makeStyles((theme) => ({
  Button: {
    margin:"5px auto",
    fontWeight:600,
     background:theme.palette.primary.blue, 
    color:theme.palette.primary.light ,

},
selectStyle: {
    minWidth: '100%'
},
paper: {
  marginTop: theme.spacing(8),
  display: 'flex',
  width: "50px",
  flexDirection: 'column',
  alignItems: 'center',
},
dialogPaper: {
  position: 'absolute',
  width: "33%",
  [theme.breakpoints.down("md")]: {
    width: "40%",
  },
  [theme.breakpoints.down("xs")]: {
    width: "80%"
},

},

}));


const RadixModal=({value,handleChange, nextStep, prevStep, openModal, closeModal})=>{
  const classes=useStyles();
 
  return(
<MuiThemeProvider>
  <Dialog classes={{paper: classes.dialogPaper }}
        open={openModal}
      fullWidth='sm'
      maxWidth='sm'
    >
    <React.Fragment>
      <DialogTitle>
        Enter Organisation Details
        <IconButton  style={{ backgroundColor: "#3f51b5",color:"#fff", float: "right" }} onClick={closeModal}  >
                        <CloseIcon />
                    </IconButton>
      </DialogTitle>
      <DialogContent>
                    <form >
                        <Grid container spacing={2} value="Container" >
                        <Grid item md={12} xs={12} sm={12}>
                                <FormControl className={classes.selectStyle} >
                                    <TextField
                                        size="large"
                                        select
                                        id="radixDepartment"
                                        label="RadixDepartment"
                                        name="radixDepartment"
                                        required
                                        onChange={handleChange('radixDepartment')}
                                        defaultValue={value.radixDepartment}
                                        // value={Priority}
                                        // onChange={(e) => setCategory(e.target.value)}
                                         >
                                        {DepartmentOptions.map((option, index) => <MenuItem key={index} value={option.value}>{option.text}</MenuItem>)}
                                    </TextField>
                                </FormControl>
                            </Grid>
                            <Grid item md={12} xs={12} sm={12}>
                                <FormControl className={classes.selectStyle}>
                                    <TextField
                                        size="large"
                                        select
                                        id='doctor'
                                        label="Doctor"
                                        name="doctor"
                                        required
                                        onChange={handleChange('doctor')}
                                        defaultValue={value.doctor}
                                        // value={Priority}
                                        // onChange={(e) => setCategory(e.target.value)}
                                         >
                                        {DepartmentOptions.map((option, index) => <MenuItem key={index} value={option.value}>{option.text}</MenuItem>)}
                                    </TextField>
                                </FormControl>
                            </Grid>
                            {/* <Grid item md={12} xs={12} sm={12}>
                                <TextField
                                    variant="outlined"
                                
                                    id="location"
                                    label="Location"
                                    name="location"
                                    onChange={handleChange('location')}
                                    defaultValue={value.location}
                                    fullWidth
                                    autoFocus
                                />
                            </Grid> */}
                        <Grid item md={6} xs={6} sm={6}>
                        <Button
                          variant="contained"
                            fullWidth
                            onClick={prevStep}
                             >
                            Back
                        </Button>
                        </Grid>
                        <Grid item md={6} xs={6} sm={6}>
                            <Button
                             variant="contained" className={classes.Button}
                            type="submit"
                            fullWidth
                            onClick={nextStep}
                            
                             >
                            Continue
                        </Button>
                        </Grid>

                        </Grid>
                    </form>
        </DialogContent>
    </React.Fragment>
  </Dialog>
</MuiThemeProvider>
  )
}



export default RadixModal;